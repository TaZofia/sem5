...
\For{$k \gets 1$ \textbf{to} $n-1$}
    \State $bound \gets \text{get\_bottom\_row}(A,k)$
    ...
    \For{$i \gets k+1$ \textbf{to} $bound$}
        ...
        \State $last \gets \text{get\_last\_column}(A,\,k + A.block\_size)$
        \For{$j \gets k+1$ \textbf{to} $last$}
...
\For{$k \gets 1$ \textbf{to} $n-1$}
  \For{$i \gets k+1$ \textbf{to} $\text{get\_bottom\_row}(LU,k)$}
...
\For{$i \gets n$ \textbf{downto} $1$}
  \State $s \gets b[i]$
  \For{$j \gets i+1$ \textbf{to} $\text{get\_last\_column}(LU,i)$}
